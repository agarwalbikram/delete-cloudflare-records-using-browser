...
return [
...[...document.querySelectorAll('a')].filter((e) => e.innerHTML === 'Delete'),
...
].filter((e) => e.innerHTML.indexOf('Edit') != -1 );
}
function confirmDelete(iteration) {
iteration = iteration || 1;
return new Promise((resolve, reject) => {
setTimeout(async () => {
let button = [...document.querySelectorAll('button')].filter((e) => e.innerHTML === 'Delete')[0];
if (button) {
button.click();
...
if ([...document.querySelectorAll('button')].filter((e) => e.innerHTML === 'Delete')[0]) {
return;
}
clearInterval(i);
resolve();
